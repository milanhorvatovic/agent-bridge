...
**Status: validation.** Written in Rust. There is no runtime yet — what exists is the CI matrix (Linux / macOS / Windows), the workspace scaffold, the first probes (`tools/pty-probe` allocates a real PTY — ConPTY on Windows — and `tools/interactive-probe` drives a real interactive CLI under one), and the first slice of the conformance corpus: `crates/fake-cli`, a deterministic scripted stand-in for an interactive CLI, with three starter scenarios under `tests/corpus/fake/` whose golden event traces run against it in CI on every commit, on all three OSes. Interrupt, resize, and cleanup probes land next; the harness runner that compares a live runtime's events against the golden traces follows with the runtime work.
