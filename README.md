...
| Reconstructed screen | **Built** — [`crates/stream`](crates/stream): a headless terminal keeps what a CLI has drawn, so a menu-rendered dialog is a dialog rather than the fragments each repaint wrote. Fed on every byte and rendered only when asked; repainted rows that say nothing new are suppressed before anything is emitted; a session whose CLI prints lines keeps no screen and pays nothing. Replayed against the recorded real-CLI fixtures at both captured widths, and against the same recordings re-cut at boundaries they never had |
