...
# replay a prior run's chronological output from its capture files
# (uses .timeline to interleave; restores per-FD routing even for merged captures)
python3 scripts/bridge.py replay <uuid> [--output-dir <path>] [--tag]
```
...
The capture is split into three files: `.out` (agent stdout, verbatim), `.err` (agent stderr, verbatim), and `.timeline` (one ASCII line per kernel chunk: `<monotonic_ns> stdout|stderr <byte_count>`) so chronological interleaving can be reconstructed if needed (in non-merge mode: `sort -n` on `.timeline`, then walk `.out`/`.err` byte offsets — or just run `bridge replay <uuid>`). For most orchestration tasks (diff, grep, cmp), `.out` and `.err` are all you'll touch. **With `merge_streams: true`** there are only two files (`.out` + `.timeline`): both streams are interleaved into `.out` in arrival order, and `bridge replay` is the way to read it back with FDs distinguished.
...
**`.timeline` semantics.** Each line is `<monotonic_ns> stdout|stderr <byte_count>` recording one `read1(4096)` chunk (one kernel read) — NOT one output line. A long agent line is split across multiple entries; many short lines collapse into one. **Sort behavior depends on capture mode:** in non-merge mode the two tee threads write timeline entries under separate locks, so lines may appear out of ts order under contention — consumers must `sort -n` to recover chronological order, and the per-stream `.out` / `.err` files each have a single writer so resorting only affects display. In **merged mode** (`merge_streams: true`) a single lock covers both the capture and timeline write, so timeline file order IS the chronological order AND matches the byte order of the merged `.out`; do NOT `sort -n` a merged-capture timeline if you plan to allocate bytes from `.out` per entry — `bridge replay` relies on file order. When in doubt, use `bridge replay <uuid>` instead of rolling your own walker.
